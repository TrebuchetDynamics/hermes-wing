...
| 1.2 | **Wing Link local runtime** — ship the signed Go host supervisor, independent acknowledged control credential, loopback plus one selected private/VPN listener, and API-first local profile topology bridge; then complete detect, adopt, verify, install, update, repair, and diagnose on Android/Termux, Linux, Windows, and macOS. Offer the pinned Donna starter profile through Hermes’s distribution installer. No remote installs or Hermes traffic proxy. | Guided local installation is the primary Android onboarding path and the desktop replacement path; Linux user-service/profile topology exists, while signed artifacts, non-Linux adapters, and a compatible Donna distribution manifest remain. |
...
- **Second domain backend** — Hermes Agent is authoritative. Wing Link is an
  independently authenticated host supervisor; its only domain bridge is
  profile topology, and it never proxies chat, session, run, or other Hermes
  traffic.
