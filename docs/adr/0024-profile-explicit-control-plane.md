...
Local and remote Navivox clients use one machine-scoped Hermes API service. Every profile-owned operation identifies its Hermes profile explicitly; API behavior must not depend on the machine’s mutable `active_profile` file or on a separate listener, port, or credential per profile.

## Consequences

- Navivox profile selection is client context and does not mutate the CLI’s active profile.
