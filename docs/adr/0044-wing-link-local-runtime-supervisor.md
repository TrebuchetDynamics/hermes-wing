...
Wing Link exposes its authenticated management API on `127.0.0.1:8654` and, after explicit pairing setup, on one selected current RFC 1918, CGNAT/Tailscale, or IPv6 ULA interface. Wildcard, public, multicast, broadcast, unspecified, and link-local listeners remain prohibited. Hermes Agent remains authoritative for runtime domain operations, while Wing Link may invoke the resolved Hermes CLI through typed, allowlisted adapters when the Agent API omits an operation. Wing Link never exposes an arbitrary CLI, shell, executable path, or config key.

Wing Link does not proxy Hermes traffic, duplicate Hermes runtime state, parse human CLI output, or embed Hermes Agent. Its initial CLI adapters cover profile topology and profile-scoped custom OpenAI-compatible provider definitions. Profile mutations use fixed create/clone/rename/delete vectors. Provider CRUD uses machine-readable `hermes --profile <id> config get providers --json` plus fixed `config set`/`config unset` paths after validating provider IDs, HTTP(S) origins, models, and revisions. Provider secrets are excluded until a typed stdin-only credential contract exists; they never enter process arguments. API errors never silently fall back to CLI. Flutter continues to send chat/session/run traffic directly to Hermes.
