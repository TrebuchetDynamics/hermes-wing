...
Wing Link exposes its authenticated management API on `127.0.0.1:8654` and, after explicit pairing setup, on one selected current RFC 1918, CGNAT/Tailscale, or IPv6 ULA interface. Wildcard, public, multicast, broadcast, unspecified, and link-local listeners remain prohibited. Hermes Agent remains authoritative for runtime domain operations. Wing Link never exposes an arbitrary CLI, shell, executable path, config key, or generic Hermes request tunnel.

Wing Link does not proxy Hermes traffic, duplicate Hermes runtime state, parse human CLI output, or embed Hermes Agent. It does not manage Hermes domain state: profiles, providers, configuration, sessions, messages, tools, schedules, approvals, and messaging-platform semantics use advertised Hermes-owned contracts directly. Missing Hermes contracts fail closed. The profile-topology and custom-provider CLI adapters prototyped after the original ADR are deprecated and are not an approved production fallback.
...
After bootstrap, Hermes Wing invokes only the fixed Wing Link executable through Termux RUN_COMMAND with allowlisted supervisor argument vectors. No arbitrary command text, client-selected executable, Hermes domain mutation, or provider secret crosses the Flutter/Kotlin boundary.
...
Wing Link may install and supervise the optional OmniRoute process for a fresh setup, but provider/model configuration uses a Hermes-owned setup contract. It does not overwrite an adopted or configured Hermes runtime. OmniRoute failure leaves a healthy Hermes installation available for normal provider setup.
...
- Prototype Wing Link profile/provider adapters must be removed or quarantined; their existence and tests do not establish an approved product capability.
