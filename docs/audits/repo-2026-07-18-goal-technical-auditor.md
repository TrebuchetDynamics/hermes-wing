...
| F-002 | Medium | fixed | Contain gateway directory startup and refresh failures | lib/features/hermes_chat/gateways/hermes_gateway_directory.dart:122-182 marked startup/refresh state before unguarded cache/store operations; lines 427-442 discarded periodic/resume refresh futures. Fixed 2026-07-29: refresh() guards the saved-gateway read and always clears the refresh indicator, _saveContacts() contains every contact-cache write, and GatewayContactCache.load() guards the preferences read. Regression cover in test/features/hermes_chat/gateways/hermes_gateway_directory_test.dart. The blocking stash is now stash@{4} after unrelated stash drops. | 1bf77fb |
...
| F-005 | Medium | resolved externally | Dependency review is knowingly red on every pull request | The blocker no longer reproduces. Verified 2026-07-29: `gh pr checks` reports `Dependency and license review` as SUCCESS on open PRs 12, 13, and 14, and .github/workflows/hermes-platform-smoke.yml:102-111 still gates the job to `pull_request`. No owner action outstanding; the runbook paragraph was refreshed. | 79f8e08 |
...
| A-003 | Medium | partly addressed | Move chat workflow state behind a deep in-process coordinator | The recommended single coordinator was not built. Six focused seams were extracted instead, each with unit cover: HermesApprovalQueue (3217ee7), HermesConnectionForm (1c38af4), attachment content rules and the StagedAttachment union (87eca42, d18f94d), HermesFollowUpQueue (87b3420), and HermesChannelObservation (351f53c). Measured 2026-07-29: _HermesChatScreenState declares 13 fields, of which 6 are handles to those seams; the remaining 7 are Riverpod handles, widget-bound Flutter controllers, and lifecycle flags that were judged not worth extracting. The nine part files and their ~6.7k lines are unchanged, so the locality problem this finding names is reduced but not resolved. The original exclusion no longer applies: the owner stashes over these files were dropped on request. | multiple |
