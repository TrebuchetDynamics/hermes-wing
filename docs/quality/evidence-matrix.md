...
| providers | Providers and models | prototype/partial | worktree-2026-08-14 | tests:runtime-model-read-only | Android, Wing Link, and Hermes API | 2026-08-14 | `/v1/models` read-only path exists; transactional new-profile setup supports allowlisted provider/model plus stdin-only credential input, while general and existing-profile provider mutation remains blocked |
