...
- `cd android && ./gradlew :app:testDebugUnitTest` — pass for Hermes Wing app native Android unit tests. The broader `./gradlew testDebugUnitTest` runs third-party plugin tests too and currently fails in `image_picker_android`'s `ImagePickerDelegateTest` in this environment.
- `npm run linux:release-build` — pass, artifact at
  `build/linux/x64/release/bundle/wing`. This container has no passwordless
...
- Manual `workflow_dispatch`: optional provider-backed Hermes web smoke when
  ...
  key in the `WING_PROVIDER_HERMES_API_KEY` repository secret.
...
When `WING_WATCH_WORKFLOW=true` (default) and the watched run succeeds, the
...
Windows/iOS/macOS native-host artifacts as recorded. If `WING_WATCH_WORKFLOW=false` is used, the helper only proves
...
```bash
WING_RUN_ANDROID_EMULATOR_SMOKE=true npm run platform:workflow-smoke
WING_RUN_PROVIDER_SMOKE=true \
WING_PROVIDER_HERMES_URL=https://hermes.example \
npm run platform:workflow-smoke
```

The provider smoke still requires the `WING_PROVIDER_HERMES_API_KEY`
...
- 2026-07-03: focused regression rerun of the since-removed `wing-e2e.spec.mjs` plus
  `hermes-smoke.spec.mjs` passed 68 Chromium tests with:
  `npx playwright test --config=playwright.config.mjs playwright/tests/regression/wing-e2e.spec.mjs playwright/tests/regression/hermes-smoke.spec.mjs --reporter=list`.
...
`HERMES_HOME`, API server enabled, and CORS for the local Hermes Wing web bundle. It
...
```bash
WING_LIVE_HERMES_PORT=8642 npm run hermes:live-smoke
WING_LIVE_HERMES_HOME=/path/to/test-home npm run hermes:live-smoke
```
...
```bash
WING_PROVIDER_HERMES_URL=http://127.0.0.1:8642 \
...
credentials, this helper starts the API server with a temporary Hermes Wing test API
key and runs the same provider smoke:

```bash
WING_CONFIGURED_HERMES_HOME=$HOME/.hermes npm run hermes:provider-smoke:local
```
...
assistant to include `WING_PROVIDER_SMOKE_OK` and
`WING_PROVIDER_VOICE_OK` by default; override the prompt/expected text with
`WING_PROVIDER_TEXT_PROMPT`, `WING_PROVIDER_TEXT_EXPECTED`,
`WING_PROVIDER_VOICE_PROMPT`, and `WING_PROVIDER_VOICE_EXPECTED` for a
...
2. Open Hermes Wing `/hermes` on web/desktop or Android.
...
```bash
WING_ANDROID_DEVICE_ID=<device-id> npm run android:voice-smoke
```
...
scripts now wait up to `WING_ANDROID_DEVICE_WAIT_SECONDS` for Flutter to list
...
```bash
WING_ANDROID_DEVICE_ID=<device-id> npm run android:hermes-voice-loop-smoke
```
...
emulator boot stabilized, `WING_ANDROID_DEVICE_ID=<emulator>
WING_ANDROID_DEVICE_WAIT_SECONDS=1 npm run android:hermes-voice-loop-smoke`
...
microphone permission, and launch Hermes Wing on an audio-capable Android target,
run:
...
WING_ANDROID_HERMES_URL=http://10.0.2.2:8642 \
npm run android:live-mic-prep
```

Current prep receipt (2026-07-03): after KVM-backed `fractal_test` emulator
boot stabilized, `WING_ANDROID_DEVICE_ID=<emulator>
WING_ANDROID_DEVICE_WAIT_SECONDS=1 WING_ANDROID_SKIP_BUILD=1
WING_ANDROID_HERMES_URL=http://10.0.2.2:8642 npm run android:live-mic-prep`
