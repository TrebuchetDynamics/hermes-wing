...
| Polish/hardening | Existing tests cover SSE reconnect/drop recovery, explicit `Accept: text/event-stream`/`Cache-Control: no-cache` stream headers, embedded event names in default SSE `message` frames, explicit JSON and non-JSON SSE error events with bounded/redacted recovery copy, offline/auth-expired copy, session search/grouping, queued follow-ups with bounded/redacted copy details, and mobile approval/error/session sheet behaviors for the implemented Hermes surfaces. Future regressions or newly wired surfaces still need focused coverage. | Covered for current implemented surfaces; not evidence for Android physical mic or server audio. |
