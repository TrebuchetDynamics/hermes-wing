...
const prompt =
  "Reply with the uppercase two-letter English greeting that rhymes with 'my'. No punctuation and no other words.";
const expected = /^hi$/i;
...
test.skip(!liveUrl, "Set WING_LIVE_HERMES_URL to run live Hermes say-hi smoke");

test("Hermes Wing sends a say-hi turn through the live Hermes Agent", async ({
  page,
}) => {
  ...
  await expect(
    semanticLabel(page, "Hermes could not finish"),
  ).not.toBeVisible();
});
