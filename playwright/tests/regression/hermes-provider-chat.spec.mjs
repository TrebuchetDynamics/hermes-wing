...
test("Hermes provider-backed text and transcript voice turns produce assistant replies", async ({
  page,
}) => {
  ...
  await page.evaluate(
    (title) => globalThis.wingE2EHermesCreateSession(title),
    smokeSessionTitle,
  );
  await expect(
    page.getByRole("heading", { name: smokeSessionTitle }),
  ).toBeVisible({ timeout: 30000 });
