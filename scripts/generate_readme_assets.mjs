...
async function captureProductFrames(browser) {
  ...
  await seedPresentation();
  const mobile = await openWingPage(browser, { width: 390, height: 844 });
  await mobile.page.evaluate(() => {
    location.hash = "/tools";
  });
  await mobile.page.getByText("GitHub workflow skill").waitFor();
  await mobile.page.getByText("ASCII art generation").waitFor();
  await mobile.page.getByText("Default Tools").waitFor();
  ...
  await mobile.page.screenshot({
    path: path.join(assets, "showcase-mobile.png"),
    clip: { x: 0, y: 0, width: 390, height: 720 },
  });
...
<h1>Completed run on desktop. Populated inventory on mobile.</h1>
<div class="meta">ONE FLUTTER CLIENT</div>
<div class="label desktop-label">DESKTOP · GATEWAY READINESS</div>
<div class="label mobile-label">MOBILE · TOOLS</div>
...
<div class="proof"><span>COMPLETED TOOL</span><span>HEALTHY SESSION</span><span>POPULATED INVENTORY</span></div>
<div class="caption">Deterministic Hermes API fixture · generated by scripts/generate_readme_assets.mjs</div>
</body></html>`);
...
function heroSvg() {
  ...
  <title id="title">Hermes Wing — Hermes Agent sessions on every screen</title>
  <desc id="desc">A healthy Hermes session trace shows capability negotiation, completed tool activity, and a streamed reply.</desc>
  <rect width="1200" height="410" rx="26" fill="#0c0e12"/>
  <path d="M670 32V378" stroke="#23262e"/>
  <g transform="translate(58 48)">
    <text x="0" y="0" dominant-baseline="hanging" fill="#7dd3fc" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="15" font-weight="700" letter-spacing="2.2">CROSS-PLATFORM HERMES AGENT CLIENT</text>
    ...
    <text x="2" y="220" dominant-baseline="hanging" fill="#f4f4f5" font-family="-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="24" font-weight="650">Follow runs. Inspect tools. Stay in control.</text>
    <text x="2" y="260" dominant-baseline="hanging" fill="#8f96a3" font-family="-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="17">One trusted session model across Android, web, and desktop.</text>
  </g>
  ...
  <g transform="translate(58 373)">
    <text x="0" y="0" dominant-baseline="hanging" fill="#5f6672" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="13" letter-spacing="1.2">ALPHA · SOURCE BUILDS · ANDROID + DESKTOP + WEB</text>
    <text x="1088" y="0" dominant-baseline="hanging" text-anchor="end" fill="#8f96a3" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="13">HERMES REMAINS AUTHORITATIVE</text>
  </g>
</svg>\n`;
}
