...
if [ -z "$device" ]; then
  ...
  if [ -z "$device" ]; then
    echo "No Android device/emulator found. Set WING_ANDROID_DEVICE_ID or start an audio-capable device/emulator." >&2
...
if [ "${WING_ANDROID_SKIP_BUILD:-0}" != "1" ]; then
...
Manual evidence still required; do not count this prep as a pass:
  ...
  2. In Hermes Wing /hermes, connect to: $(android_endpoint_hint)
