...
void main() {
  ...
  test('rejects attached tokens after unsupported connection URLs', () {
    final result = parseNavivoxConnectionImportPayload(
      'Open ftp://gateway.example/connect:Token: nvbx_unsupported.',
    );
  ...
  test(
    'rejects endpoint fragments instead of silently dropping hidden state',
    () {
...
''');
  ...
  test('parses copied URL token after attached URL colon punctuation', () {
    final result = parseNavivoxConnectionImportPayload(
  ...
  test(
    'parses copied URL token after attached closing wrapper punctuation',
    () {
      final result = parseNavivoxConnectionImportPayload(
        'https://gateway.example/connect)Token:shared_secret',
      );
  ...
  test('parses copied URL token after rich-text non-breaking space', () {
    final result = parseNavivoxConnectionImportPayload(
      'https://gateway.example/connect\u00A0Token: shared_secret',
    );
