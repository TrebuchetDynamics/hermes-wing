...
public class SpeechToTextPlugin :
    ...
    private fun startListening(result: Result, languageTag: String, partialResults: Boolean,
        ...
        handler.post {
            if (session != recognitionSession) return@post
    ...
    private fun stopListening(result: Result) {
        ...
        val recognizerToStop = speechRecognizer
        handler.post {
            recognizerToStop?.stopListening()
            if (!recognizerStops && speechRecognizer === recognizerToStop) {
                destroyRecognizer()
    ...
    private fun cancelListening(result: Result) {
        ...
        val recognizerToCancel = speechRecognizer
        handler.post {
            recognizerToCancel?.cancel()
            if (!recognizerStops && speechRecognizer === recognizerToCancel) {
                destroyRecognizer()
                notifyListening(isRecording = false)
            }
        }
...
//            var recognizerIntent = Intent(RecognizerIntent.ACTION_GET_LANGUAGE_DETAILS)
    ...
    private inner class SessionRecognitionListener(
        ...
        private fun current(block: () -> Unit) {
            if (session == recognitionSession) block()
        }

        override fun onReadyForSpeech(params: Bundle?) = current {
            this@SpeechToTextPlugin.onReadyForSpeech(params)
        }

        override fun onBeginningOfSpeech() = current {
            this@SpeechToTextPlugin.onBeginningOfSpeech()
        }

        override fun onRmsChanged(rmsdB: Float) = current {
            this@SpeechToTextPlugin.onRmsChanged(rmsdB)
        }

        override fun onBufferReceived(buffer: ByteArray?) = current {
            this@SpeechToTextPlugin.onBufferReceived(buffer)
        }

        override fun onEndOfSpeech() = current {
            this@SpeechToTextPlugin.onEndOfSpeech()
        }

        override fun onError(error: Int) = current {
            this@SpeechToTextPlugin.onError(error)
        }

        override fun onResults(results: Bundle?) = current {
            this@SpeechToTextPlugin.onResults(results)
        }

        override fun onPartialResults(partialResults: Bundle?) = current {
            this@SpeechToTextPlugin.onPartialResults(partialResults)
        }

        override fun onEvent(eventType: Int, params: Bundle?) = current {
            this@SpeechToTextPlugin.onEvent(eventType, params)
        }
    }
    ...
    private fun createRecognizer(onDevice: Boolean, listenMode: ListenMode, pauseFor: Int?) {
        ...
        val session = recognitionSession
        val sessionListener = SessionRecognitionListener(session)
        speechRecognizer?.destroy()
        speechRecognizer = null
        handler.post {
            run {
                debugLog("Creating recognizer for session $session")
                if (session != recognitionSession) return@run
                if (intentLookup) {
                    speechRecognizer = createSpeechRecognizer(
                        pluginContext,
                        pluginContext?.findComponentName()
                    ).apply {
                        debugLog("Setting generation-bound listener after intent lookup")
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && onDevice) {
                        ...
                        if (supportsLocal ) {
                            speechRecognizer = createOnDeviceSpeechRecognizer(pluginContext!!).apply {
                                debugLog("Setting generation-bound on device listener")
                                setRecognitionListener(sessionListener)
                            }
                        }
                    }
                    if ( null == speechRecognizer) {
                        speechRecognizer = createSpeechRecognizer(pluginContext).apply {
                            debugLog("Setting generation-bound default listener")
                            setRecognitionListener(sessionListener)
                        }
                    }
                }
    ...
    private fun destroyRecognizer() {
        ...
        if (speechRecognizer === recognizerToDestroy) {
            speechRecognizer = null
        }
