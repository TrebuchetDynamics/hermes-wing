...
class SpeechToText {
  ...
  int _listenHandoffGeneration = 0;
  int? _pendingListenHandoff;
  ...
  Future<void> _stop() async {
    ...
    _listenHandoffGeneration += 1;
    _pendingListenHandoff = null;
  ...
  Future listen(
    ...
    if (_pendingListenHandoff != null || isListening) {
      throw ListenFailedException('listen already active', null, null);
    }
    final handoffGeneration = ++_listenHandoffGeneration;
    _pendingListenHandoff = handoffGeneration;
    _lastError = null;
    _lastRecognized = '';
    _userEnded = false;
    ...
    try {
      ...
      if (_pendingListenHandoff != handoffGeneration) return;
      _pendingListenHandoff = null;
      if (started) {
        // The native Android method publishes listening before completing its
        // method result. Re-publish it only after the generation handoff is
        // complete so predecessor callbacks cannot reach successor listeners.
        _onNotifyStatus(listeningStatus);
      ...
      if (_pendingListenHandoff == handoffGeneration) {
        _pendingListenHandoff = null;
      }
  ...
  void _onTextRecognition(String resultJson) {
    if (_pendingListenHandoff != null) return;
