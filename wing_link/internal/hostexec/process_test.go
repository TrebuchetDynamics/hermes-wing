...
func TestProcessWritesSecretOnlyToStdin(t *testing.T) {
	const secret = "provider-secret-never-in-argv-env-or-output"
	spec := helperCommand("stdin")
	spec.Input = []byte(secret + "\n")
	var lines []string
	result := runProcess(context.Background(), spec, func(line string) {
		lines = append(lines, line)
	})
	...
	if got != fmt.Sprintf("stdin_bytes=%d", len(secret)+1) || strings.Contains(got, secret) {
		t.Fatalf("unsafe or incomplete stdin result: %q", got)
	}
	if strings.Contains(strings.Join(spec.Args, "\n"), secret) || strings.Contains(strings.Join(spec.Env, "\n"), secret) {
		t.Fatal("secret entered argv or environment")
	}
}
